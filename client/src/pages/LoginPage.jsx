...
function LoginPage() {
  ...
  return (
    ...
    max-sm:flex-col backdrop:blur-2xl'>
      ...
      {/* right */}
      <form onSubmit={onSubmitHandler} className="border-2 bg-white/8
      text-white border-gray-500 p-6 
      flex flex-col gap-6 rounded-lg shadow-lg">
        ...
        items-center'>
          {currState}
          {isDataSubmitted && 
          <img onClick={()=>setIsDataSubmitted(false)} src={assets.arrow_icon} alt="" 
          className='w-5 cursor-pointer'/>}
          </h2>
          {currState==="Sign Up"&&!isDataSubmitted&&(
          <input onChange={(e)=>setFullName(e.target.value)} 
              value={fullName} type="text" name=''
          ...
          <div className='flex flex-col gap-2'>
            {currState==="Sign Up"?(
              ...
              }className='font-medium text-violet-600 cursor-pointer'>Click here</span></p>
            )}
          </div>
      </form>
    </div>
